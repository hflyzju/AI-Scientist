...
def save_json_data_to_file(file_path: str, data: dict):
...
def perform_review(
    ...
    if num_fs_examples > 0:
        if args is not None and args.use_only_problem_and_method_for_review:
            fs_prompt = get_review_fewshot_examples(num_fs_examples, use_only_problem_and_method=True)
        else:
            fs_prompt = get_review_fewshot_examples(num_fs_examples, use_only_problem_and_method=False)
...
def get_review_fewshot_examples(num_fs_examples=1, use_only_problem_and_method=False):
    fewshot_prompt = """
...
"""
    for paper, review in zip(
        ...
        logger.info(f"get_review_fewshot_examples | use_only_problem_and_method: {use_only_problem_and_method}")
        if not use_only_problem_and_method:
